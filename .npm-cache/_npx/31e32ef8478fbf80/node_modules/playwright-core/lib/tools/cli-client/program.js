...
`);
  ...
  switch (commandName) {
    ...
    case "attach": {
      ...
      const attachSessionName = (0, import_registry.explicitSessionName)(args.session) ?? attachTarget ?? sessionName;
...
async function listSessions(registry, clientInfo, all) {
  ...
  for (const [workspaceKey, list] of entries) {
    if (!all && workspaceKey !== key)
      continue;
    count += await gcAndPrintSessions(clientInfo, list.map((entry) => new import_session.Session(entry)), all ? `${import_path.default.relative(process.cwd(), workspaceKey) || "/"}:` : void 0, runningSessions);
  }
