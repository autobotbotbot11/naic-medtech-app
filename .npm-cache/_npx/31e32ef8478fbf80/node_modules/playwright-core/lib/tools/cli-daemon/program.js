...
var import_env = require("../../server/utils/env");
...
async function ensureConfiguredBrowserInstalled() {
  if ((0, import_env.getAsBooleanFromENV)("PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD"))
    return;
...
async function resolveAndInstall(nameOrChannel) {
  const executables = import_registry2.registry.resolveBrowsers([nameOrChannel], { shell: "no" });
  await import_registry2.registry.install(executables);
}
