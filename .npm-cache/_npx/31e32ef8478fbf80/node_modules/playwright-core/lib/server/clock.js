...
class Clock {
  ...
  async fastForward(ticks) {
    ...
    this._initScripts.push(await this._browserContext.addInitScript(import_progress.nullProgress, `globalThis.__pwClock.controller.log('fastForward', ${Date.now()}, ${ticksMillis})`));
  ...
  async install(time) {
    ...
    this._initScripts.push(await this._browserContext.addInitScript(import_progress.nullProgress, `globalThis.__pwClock.controller.log('install', ${Date.now()}, ${timeMillis})`));
    await this._evaluateInFrames(`globalThis.__pwClock.controller.install(${timeMillis})`);
  }
  async pauseAt(ticks) {
    await this._installIfNeeded();
    const timeMillis = parseTime(ticks);
    this._initScripts.push(await this._browserContext.addInitScript(import_progress.nullProgress, `globalThis.__pwClock.controller.log('pauseAt', ${Date.now()}, ${timeMillis})`));
  ...
  resumeNoReply() {
    ...
    const doResume = async () => {
      this._initScripts.push(await this._browserContext.addInitScript(import_progress.nullProgress, `globalThis.__pwClock.controller.log('resume', ${Date.now()})`));
  ...
  async setFixedTime(time) {
    ...
    this._initScripts.push(await this._browserContext.addInitScript(import_progress.nullProgress, `globalThis.__pwClock.controller.log('setFixedTime', ${Date.now()}, ${timeMillis})`));
    await this._evaluateInFrames(`globalThis.__pwClock.controller.setFixedTime(${timeMillis})`);
  }
  async setSystemTime(time) {
    await this._installIfNeeded();
    const timeMillis = parseTime(time);
    this._initScripts.push(await this._browserContext.addInitScript(import_progress.nullProgress, `globalThis.__pwClock.controller.log('setSystemTime', ${Date.now()}, ${timeMillis})`));
    await this._evaluateInFrames(`globalThis.__pwClock.controller.setSystemTime(${timeMillis})`);
  }
  async runFor(ticks) {
    await this._installIfNeeded();
    const ticksMillis = parseTicks(ticks);
    this._initScripts.push(await this._browserContext.addInitScript(import_progress.nullProgress, `globalThis.__pwClock.controller.log('runFor', ${Date.now()}, ${ticksMillis})`));
  ...
  async _installIfNeeded() {
    ...
    const initScript = await this._browserContext.addInitScript(import_progress.nullProgress, script);
