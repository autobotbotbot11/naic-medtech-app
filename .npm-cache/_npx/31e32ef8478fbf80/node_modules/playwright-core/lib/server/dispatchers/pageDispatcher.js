...
class PageDispatcher extends import_dispatcher.Dispatcher {
  ...
  _onDispose() {
    ...
    this._page.setFileChooserInterceptedBy(import_progress.nullProgress, false, this).catch(() => {
    });
...
class WorkerDispatcher extends import_dispatcher.Dispatcher {
  ...
  async evaluateExpression(params, progress) {
    return { value: (0, import_jsHandleDispatcher.serializeResult)(await this._object.evaluateExpression(progress, params.expression, params.isFunction, (0, import_jsHandleDispatcher.parseArgument)(params.arg))) };
  }
  async evaluateExpressionHandle(params, progress) {
    return { handle: import_jsHandleDispatcher.JSHandleDispatcher.fromJSHandle(this, await this._object.evaluateExpressionHandle(progress, params.expression, params.isFunction, (0, import_jsHandleDispatcher.parseArgument)(params.arg))) };
  }
  async updateSubscription(params, progress) {
    if (params.enabled)
      this._subscriptions.add(params.event);
