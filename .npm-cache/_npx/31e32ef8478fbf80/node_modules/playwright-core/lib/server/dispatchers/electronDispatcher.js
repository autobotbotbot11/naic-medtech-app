...
class ElectronApplicationDispatcher extends import_dispatcher.Dispatcher {
  ...
  async browserWindow(params, progress) {
    const handle = await this._object.browserWindow(progress, params.page.page());
  ...
  async evaluateExpression(params, progress) {
    ...
    return { value: (0, import_jsHandleDispatcher.serializeResult)(await handle.evaluateExpression(progress, params.expression, { isFunction: params.isFunction }, (0, import_jsHandleDispatcher.parseArgument)(params.arg))) };
  }
  async evaluateExpressionHandle(params, progress) {
    const handle = await progress.race(this._object._nodeElectronHandlePromise);
    const result = await handle.evaluateExpressionHandle(progress, params.expression, { isFunction: params.isFunction }, (0, import_jsHandleDispatcher.parseArgument)(params.arg));
