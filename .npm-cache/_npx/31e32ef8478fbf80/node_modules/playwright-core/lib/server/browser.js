...
class Browser extends import_instrumentation.SdkObject {
  ...
  downloadCreated(page, uuid, url, suggestedFilename, downloadFilename) {
  ...
  async startServer(progress, title, options) {
    return await progress.race(this._server.start(title, options));
  }
  async stopServer(progress) {
  ...
  didClose() {
    ...
    this.stopServer(import_progress.nullProgress).catch(() => {
    });
