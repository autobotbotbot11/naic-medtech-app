...
class BrowserContext extends import_instrumentation.SdkObject {
  ...
  async initialize() {
    ...
    if (this._options.serviceWorkers === "block")
      await this.addInitScript(import_progress.nullProgress, `
...
`);
  ...
  async exposeConsoleApi(progress) {
  ...
  setHTTPCredentials(progress, httpCredentials) {
  ...
  async loadDefaultContextAsIs(progress) {
  ...
  async loadDefaultContext(progress) {
  ...
  async _internalAddInitScript(source) {
    ...
    if (originsToSave.size) {
      ...
      try {
        ...
        for (const origin of originsToSave) {
          ...
          const storage = await frame.evaluateExpression(progress, collectScript, { world: "utility" });
          if (storage.localStorage.length || storage.indexedDB?.length)
            result.origins.push({ origin, localStorage: storage.localStorage, indexedDB: storage.indexedDB });
        }
      } finally {
...
` + error.message);
  ...
  async harExport(progress, harId) {
