...
class ElementHandle extends js.JSHandle {
  ...
  async _clickablePoint(progress) {
  ...
  async _offsetPoint(progress, offset) {
  ...
  async _performPointerAction(progress, actionName, waitForEnabled, action, forceScrollOptions, options) {
    ...
    if (force) {
      ...
      const handle = await progress.race(this._evaluateHandleInUtility(([injected, node, { actionType: actionType2, hitPoint: hitPoint2, trial }]) => injected.setupHitTargetInterceptor(node, actionType2, hitPoint2, trial), { actionType, hitPoint, trial: !!options.trial }));
  ...
  async _beforeNonPointerAction(progress) {
    if (progress.metadata.annotate)
      progress.metadata.box = await this.boundingBox(progress) || void 0;
    await progress.race(this.instrumentation.onBeforeInputAction(this, progress.metadata));
  }
  ...
  async _setInputFiles(progress, items) {
    ...
    const result = await progress.race(this._evaluateHandleInUtility(([injected, node, { multiple: multiple2, directoryUpload }]) => {
  ...
  async querySelector(progress, selector, options) {
    return progress.race(this._querySelector(selector, options));
  }
  async _querySelector(selector, options) {
    return this._frame.selectors.query(selector, options, this);
  }
  async querySelectorAll(progress, selector) {
  ...
  async evalOnSelector(progress, selector, strict, expression, isFunction, arg) {
    return this._frame.evalOnSelector(progress, selector, strict, expression, isFunction, arg, this);
  }
  async evalOnSelectorAll(progress, selector, expression, isFunction, arg) {
  ...
  async _checkFrameIsHitTarget(progress, point) {
    let frame = this._frame;
    const data = [];
    while (frame.parentFrame()) {
      const frameElement = await frame.frameElement(progress);
      const box = await frameElement.boundingBox(progress);
      const style = await progress.race(frameElement.evaluateInUtility(([injected, iframe]) => injected.describeIFrameStyle(iframe), {}).catch((e) => "error:notconnected"));
    ...
    for (let i = data.length - 1; i > 0; i--) {
      ...
      const hitTargetResult = await progress.race(element.evaluateInUtility(([injected, element2, hitPoint]) => {
