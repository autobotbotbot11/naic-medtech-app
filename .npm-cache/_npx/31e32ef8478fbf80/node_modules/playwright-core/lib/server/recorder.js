...
class Recorder extends import_events.default {
  ...
  async _install() {
    ...
    await controller.run(async (progress) => {
      ...
      await this._context.exposeBinding(progress, "__pw_recorderElementPicked", false, async ({ frame }, elementInfo) => {
        const selectorChain = await (0, import_recorderUtils.generateFrameSelector)(progress, frame);
      ...
      await this._context.exposeBinding(
        ...
        false,
        (source, action) => this._performAction(progress, source.frame, action)
      );
      await this._context.exposeBinding(
        progress,
        "__pw_recorderRecordAction",
        false,
        (source, action) => this._recordAction(progress, source.frame, action)
      );
  ...
  async setMode(mode) {
    ...
    if (this._mode !== "none" && this._mode !== "standby") {
      ...
      pageToFocus?.bringToFront(import_progress.nullProgress).catch(() => {
      });
    }
  ...
  async _describeFrame(progress, frame) {
    return {
      pageGuid: frame._page.guid,
      pageAlias: this._pageAliases.get(frame._page),
      framePath: await (0, import_recorderUtils.generateFrameSelector)(progress, frame)
    };
  }
  _testIdAttributeName() {
    return this._params.testIdAttributeName || this._context.selectors().testIdAttributeName() || "data-testid";
  }
  async _createActionInContext(progress, frame, action) {
  ...
  async _performAction(progress, frame, action) {
    ...
    if (actionInContext.action.name !== "openPage" && actionInContext.action.name !== "closePage")
      await (0, import_recorderRunner.performAction)(progress, this._pageAliases, actionInContext);
    actionInContext.endTime = (0, import_utils2.monotonicTime)();
  }
  async _recordAction(progress, frame, action) {
    const actionInContext = await this._createActionInContext(progress, frame, action);
    this._signalProcessor.addAction(actionInContext);
  }
