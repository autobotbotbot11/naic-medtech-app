...
class RecorderApp {
  ...
  async _init(inspectedContext) {
    ...
    await controller.run(async (progress) => {
      ...
      this._page.once("close", () => {
        ...
        this._page.browserContext.close(import_progress.nullProgress, { reason: "Recorder window closed" }).catch(() => {
        });
  ...
  async close() {
    await this._page.close(import_progress.nullProgress);
  }
  ...
  _wireListeners(recorder) {
    ...
    recorder.on(import_recorder.RecorderEvent.ElementPicked, (elementInfo, userGesture) => {
      if (userGesture)
        this._page.bringToFront(import_progress.nullProgress).catch(() => {
        });
  ...
  return new Proxy({}, {
    get: (_target, prop) => {
      if (typeof prop !== "string")
        return void 0;
      return (params) => {
        page.mainFrame().evaluateExpression(import_progress.nullProgress, ((event) => {
