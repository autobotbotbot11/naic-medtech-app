...
class HarTracer {
  ...
  _onDOMContentLoaded(page, pageEntry) {
    const promise = page.mainFrame().evaluateExpression(import_progress.nullProgress, String(() => {
      return {
  ...
  async _onRequestFinished(request, response) {
    ...
    const page = request.frame()?._page;
    if (!this._options.omitServerIP) {
      this._addBarrier(page || request.serviceWorker(), response.serverAddr(import_progress.nullProgress).then((server) => {
        if (server?.ipAddress)
    ...
    if (!this._options.omitSecurityDetails) {
      this._addBarrier(page || request.serviceWorker(), response.securityDetails(import_progress.nullProgress).then((details) => {
        if (details)
    ...
    this._addBarrier(page || request.serviceWorker(), response.httpVersion(import_progress.nullProgress).then((httpVersion) => {
    ...
    this._computeHarEntryTotalTime(harEntry);
    if (!this._options.omitSizes) {
      this._addBarrier(page || request.serviceWorker(), response.sizes(import_progress.nullProgress).then((sizes) => {
  ...
  _onResponse(response) {
    ...
    this._recordRequestOverrides(harEntry, request);
    this._addBarrier(page || request.serviceWorker(), request.rawRequestHeaders(import_progress.nullProgress).then((headers) => {
      this._recordRequestHeadersAndCookies(harEntry, headers);
    }));
    this._recordResponseHeaders(harEntry, response.headers());
    this._addBarrier(page || request.serviceWorker(), response.rawResponseHeaders(import_progress.nullProgress).then((headers) => {
