...
class Frame extends import_instrumentation.SdkObject {
  ...
  async waitForNavigation(progress, requiresNewDocument, options) {
    const waitUntil = verifyLifecycle("waitUntil", options.waitUntil === void 0 ? "load" : options.waitUntil);
  ...
  async evalOnSelector(progress, selector, strict, expression, isFunction, arg, scope) {
    return progress.race(this._evalOnSelector(selector, strict, expression, isFunction, arg, scope));
  }
  async _evalOnSelector(selector, strict, expression, isFunction, arg, scope) {
  ...
  async evalOnSelectorAll(progress, selector, expression, isFunction, arg, scope) {
    return progress.race(this._evalOnSelectorAll(selector, expression, isFunction, arg, scope));
  }
  async _evalOnSelectorAll(selector, expression, isFunction, arg, scope) {
  ...
  async queryCount(progress, selector, options) {
    try {
  ...
  async addScriptTag(progress, params) {
    return await progress.race(this._addScriptTag(params));
  }
  async _addScriptTag(params) {
    const {
      url = null,
      content = null,
      type = ""
  ...
  async addStyleTag(progress, params) {
    return await progress.race(this._addStyleTag(params));
  }
  async _addStyleTag(params) {
    const {
      url = null,
      content = null
    } = params;
