...
${uaError}` };
  ...
  async _initReuseBrowsersMode(browserName, launchOptions, id) {
    ...
    for (const b of this._playwright.allBrowsers()) {
      ...
      if (b.options.name === browserName && b.options.channel === launchOptions.channel)
        await b.close(import_progress.nullProgress, { reason: "Connection terminated" });
    }
    if (!browser) {
      ...
      dispose: async () => {
        for (const context of browser.contexts()) {
          if (!context.pages().length)
            await context.close(import_progress.nullProgress, { reason: "Connection terminated" });
        }
      }
    };
  }
  ...
  async _initLaunchBrowserMode(browserName, proxyValue, launchOptions, id) {
    ...
    return {
      ...
      denyLaunch: true,
      dispose: async () => {
        await browser.close(import_progress.nullProgress, { reason: "Connection terminated" });
        socksProxy?.close();
      }
    };
  }
